...
def mainfunc():
	...
	while True:
		...
		def lighton( LID, LON, SCE ):
			...
			if LON == 0:
				on = {"on":True}
				u = requests.put(L, json.dumps(on), timeout=5)
				with con:
					cur = con.cursor()
		...
		if (GPIO.input(16) == 1):
			Counter += 1
			if Counter == 1:
				light_on1 = lighton( L1ID, L1ON, scene1 )
				light_on2 = lighton( L2ID, L2ON, scene1 )
				light_on3 = lighton( L3ID, L3ON, scene1 )
				light_on5 = lighton( L5ID, L5ON, scene1 )
			elif Counter == 2:
				light_on1 = lighton( L1ID, L1ON, scene2 )
				light_on2 = lighton( L2ID, L2ON, scene2 )
				light_on3 = lighton( L3ID, L3ON, scene2 )
				light_on5 = lighton( L5ID, L5ON, scene2 )
			elif Counter == 3:
				light_on1 = lighton( L1ID, L1ON, scene3 )
				light_on2 = lighton( L2ID, L2ON, scene3 )
				light_on3 = lighton( L3ID, L3ON, scene3 )
				light_on5 = lighton( L5ID, L5ON, scene3 )
			elif Counter == 4:
				light_on1 = lighton( L1ID, L1ON, scene4 )
				light_on2 = lighton( L2ID, L2ON, scene4 )
				light_on3 = lighton( L3ID, L3ON, scene4 )
				light_on5 = lighton( L5ID, L5ON, scene4 )
				Counter = 0
			sleep(ButtonSleep)
